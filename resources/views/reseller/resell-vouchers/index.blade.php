...
<div class="admin-actions">
    <div>
        ...
        <div class="admin-page-subtitle">
            Validate and manage assigned vouchers
        </div>
    </div>
</div>
...
<form method="GET" action="{{ route('reseller.resell-vouchers.index') }}">
    ...
    <div style="text-align:right; margin-top:8px;">
        <a href="{{ route('reseller.resell-vouchers.index') }}"
    ...
    <table class="admin-table">
        ...
        @forelse($vouchers as $voucher)
            <tr>
                ...
                <td>
                    {{ $voucher->expires_at?->format('Y-m-d') ?? 'N/A' }}
                </td>
                ...
                <td style="text-align:right">
                    ...
                    @if($voucher->status === \App\Enums\VoucherStatus::DEACTIVATE)
                        ...
                        {{-- Validate Only --}}
                        <form action="{{ route('reseller.resell-vouchers.validate', $voucher) }}"
                              method="POST"
                              style="display:inline">
                            @csrf
                            <button class="admin-btn admin-btn-sm admin-btn-primary"
                                    onclick="return confirm('Activate this voucher?')">
                                ✅ Activate
                            </button>
                        </form>
...
{{ $vouchers->links('reseller.partials.pagination') }}
